...
async function main(){
  ...
  for (const [tk, t] of [["pink", THEMES.pink]]) {
    ...
    await render(genTop10HTML(total.slice(0, topN), 'TOP ' + topN, t), path.join(outputDir, `thanks_rank_${suff}_p1.jpg`));
    const pages = [
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await render(genHTML(total.slice(p.start, p.end), p.label, t), path.join(outputDir, `thanks_rank_${suff}_p${i + 2}.jpg`));
    }
  }
  console.log('\n✅ Pink theme done!');
}
